...
function LandingScreen({ onGetStarted, onHowItWorks }) {
    ...
    <div style={{ minHeight: '100vh', background: COLORS.bg, overflowX: 'hidden' }}>
      ...
      <div style={{ textAlign: 'center', padding: '60px 64px 0', position: 'relative' }}>
        ...
        <div style={{ position: 'relative', width: 280, height: 280, margin: '0 auto 48px', ...fade(100) }}>
          <style>{`
            ...
            @keyframes tick-in { from { transform: scale(0) rotate(-45deg); opacity: 0; } to { transform: scale(1) rotate(0deg); opacity: 1; } }
            @keyframes confetti-fall { from { transform: translateY(-20px) rotate(0deg); opacity: 1; } to { transform: translateY(600px) rotate(720deg); opacity: 0; } }
            @keyframes deploy-glow { 0%, 100% { box-shadow: 0 0 20px rgba(45,212,191,0.3); } 50% { box-shadow: 0 0 40px rgba(45,212,191,0.6); } }
            @keyframes fade-cycle { 0%, 20% { opacity: 1; } 25%, 95% { opacity: 0; } 100% { opacity: 1; } }
          `}</style>
        ...
        {/* Headline */}
        <h1 style={{ fontSize: 72, fontWeight: 800, lineHeight: 1.05, letterSpacing: '-0.03em', margin: '0 0 24px', ...fade(200),
          ...
          animation: 'shimmer 5s linear infinite', fontFamily: "Ubuntu"
        }}>
          Your AI.<br />Completely private.
        </h1>

        <p style={{ color: COLORS.textSecondary, fontSize: 20, lineHeight: 1.6, maxWidth: 560, margin: '0 auto 40px', fontWeight: 400, ...fade(250) }}>Chat with powerful AI that runs on your own private cloud. No one can read your conversations! Not Us, Not Anyone. Ready in minutes, no tech skills needed.
        ...
        <div style={{ display: 'flex', gap: 14, justifyContent: 'center', marginBottom: 20, ...fade(300) }}>
          ...
          <GhostButton onClick={onHowItWorks} style={{ fontSize: 16, padding: '16px 32px' }}>
            See how it works
          </GhostButton>
        </div>
      ...
      <div id="how-it-works" style={{ padding: '100px 64px 80px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 60, ...fade(450) }}>
          ...
          <h2 style={{ fontFamily: 'Syne, sans-serif', fontSize: 44, fontWeight: 700, color: COLORS.textPrimary, margin: '12px 0 0', letterSpacing: '-0.02em' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 24, ...fade(500) }}>
          {[
          {
            ...
            desc: 'Pick from Azure, AWS, or Google Cloud. You already have an account, or we\'ll walk you through creating one for free.',
            icon:
          ...
          <Card key={num} style={{ padding: 28 }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div>
                  ...
                  <h3 style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 700, color: COLORS.textPrimary, margin: '4px 0 8px', letterSpacing: '-0.01em' }}>{title}</h3>
        ...
        <div style={{ textAlign: 'center', marginTop: 56, ...fade(550) }}>
          <PrimaryButton onClick={onGetStarted} size="lg">
            Get started. It's easy
          </PrimaryButton>
        </div>
      </div>
